...
export const getPromoByDate = async (req: AuthRequest,res: Response) => {
    ...
    res.status(202).json({message: 'Successfully Load Promo Of Date: '+date, data: promos.length});
    return;
}


export const getTimeByDate = async (req: AuthRequest,res: Response) => {
    ...
    for (let i = 0; i < promos.length; i++) {
        console.log(promos[i].time);
        sum+=Number(promos[i].time);
    }

    res.status(202).json({message: 'Successfully Load Total Time Of Date: '+date, data: sum});
    return;
}