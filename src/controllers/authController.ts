...
export const changePassword = async (req: Request, res: Response) => {
    ...
    if (!email || !password) {
        res.status(400).json({ message: 'Email and New Password Require to change the password!', data: null });
        return;
    }
    ...
    try {
        ...
        const result = await User.updateOne(filter1,{password: hashedPassword});

        if(result){
            res.status(202).json({ message: 'Password changed succesfully!', data: null });
        }
    }
    catch (e) {
        res.status(500).json({ message: 'Server error!', data: null });
